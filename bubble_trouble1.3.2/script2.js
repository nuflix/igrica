...
let audio = new Audio("lofi_music.mp3");
...
function draw(animationTime) {
    ...
    if(ballsY[i]+80  > canvas.height - ballsRadius[i]) {
        dy[i] = -Math.abs(dy[i]);
    }else if(ballsY[i] < ballsRadius[i]){
        dy[i] = Math.abs(dy[i]); 
    }
    // dio kojim se provjerava da li je loptica udarila paddle
    else if(ballsY[i] + ballsRadius[i] -4 > canvas.height - paddleHeight + dy[i]/20 * deltaTime) {
